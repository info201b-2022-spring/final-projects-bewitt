...
continuous_data <- select(heart_data, BMI, PhysicalHealth, MentalHealth)
...
intro_page <- tabPanel(
  ...
  img("heart image two", src = "image2.jpeg"),
...
server <- function(input, output) {
  ...
  make_hist_df <- function(var_name) {
    hist_df_whd <- filter(heart_data, HeartDisease == 1)
    hist_df_wohd <- filter(heart_data, HeartDisease == 1)
  }
  
  output$hist <- renderPlot({
    ggplot(heart_data, aes(x = .data[[input$continuous]], fill = .data[[HeartDisease]], color = .data[[HeartDisease]])) +
      geom_histogram(position = "identity")
  })
  
  output$testing <- renderText({
    paste0(input$continuous)
  })
